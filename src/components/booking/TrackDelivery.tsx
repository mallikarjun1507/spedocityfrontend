...
export function TrackDelivery() {
  ...
  useEffect(() => {
    if (!bookingData) navigate("/dashboard");
  ...
  // Auto navigate to map tracking after short delay
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-b from-blue-50 to-white flex items-center justify-center px-4">
      <motion.div
        initial={{ scale: 0.8, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ type: "spring", damping: 20, stiffness: 300 }}
        className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full text-center shadow-2xl relative"
      >
        {/* ✅ Success Checkmark */}
        <motion.div
          initial={{ scale: 0 }}
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => navigate("/active-order", { state: bookingData })}
